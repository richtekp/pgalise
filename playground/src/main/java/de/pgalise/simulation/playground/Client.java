...
public class Client {
	...
	public static void main(String args[]) throws NamingException, IOException, IllegalStateException,
			InitializationException, ClassNotFoundException, SQLException, SensorException {
		...
		InitParameter initParameter = new InitParameter(cityInfrastructure, produceServerConfiguration(),
				SIMULATION_START.getTimeInMillis(), SIMULATION_END.getTimeInMillis(), SIMULATION_INTERVAL_IN_MILLIES,
				SIMULATION_CLOCKTIME_INTERVAL_IN_MILLIES, OC_SERVLET_URL, CC_SERVLET_URL, new TrafficFuzzyData(1, 0.9,
						1), cityInfrastructure.getBoundary());
		...
		log.debug("Finished creating EventList, elapsed time: " + (System.currentTimeMillis() - startTime) / 1000 / 60);
	 ...
	 */
	private static Node getRandomGeoLocation(CityInfrastructureData city, Random random) {
	 ...
	 * @return
	 */
	private static List<SimulationEventList> produceSimulationEventLists(CityInfrastructureData cityInfrastructureData,
		...
		CreateRandomVehicleData data = new CreateRandomVehicleData(sensorLists, new VehicleInformation(id, true,
				VehicleTypeEnum.CAR, VehicleModelEnum.CAR_BMW_1, trip, "K.I.T.T."));
		...
		// start
		// +1min
		data = new CreateRandomVehicleData(sensorLists, new VehicleInformation(id, true, VehicleTypeEnum.CAR,
				VehicleModelEnum.CAR_BMW_1, trip, "C.A.R."));
		vehicleDataList.add(data);
		...
		// +1min
		data = new CreateRandomVehicleData(sensorLists, new VehicleInformation(id, true, VehicleTypeEnum.CAR,
				VehicleModelEnum.CAR_BMW_1, trip, "Ente"));
		vehicleDataList.add(data);
	...
	private static int getUniqueSensorID() {
		Random random = new Random(RANDOM_SEED_SERVICE.getSeed(Client.class.getName()));

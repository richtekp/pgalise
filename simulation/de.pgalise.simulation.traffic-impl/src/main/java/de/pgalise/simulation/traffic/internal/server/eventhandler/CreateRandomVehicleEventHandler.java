 ...
 * @version 1.0
 */
public class CreateRandomVehicleEventHandler<D extends VehicleData, E extends CreateRandomVehiclesEvent<D>>
  extends AbstractVehicleEventHandler<D, E> {

  /**
   * Logger
   */
  private static final Logger log = LoggerFactory.getLogger(
    CreateRandomVehicleEventHandler.class);
  ...
  public void handleEvent(CreateRandomVehiclesEvent event) {
    ...
    log.info("Processing CREATE_RANDOM_VEHICLES_EVENT: Vehicles=" + event.
      getCreateRandomVehicleDataList().size());

    final List<CreateRandomVehicleData> vehicleList = event.
    ...
    for (int i = 0; i < createThreads.length - 1; i++) {
      createThreads[i] = new CreateThread(this,
        vehicleList,
        i * listPartSize,
        i * listPartSize + listPartSize);
      createThreads[i].setPriority(Thread.MAX_PRIORITY);
    }
    createThreads[createThreads.length - 1] = new CreateThread(this,
      vehicleList,
      (createThreads.length - 1) * listPartSize,
      vehicleList.size());
  ...
  private static class CreateThread<X extends VehicleData, Y extends CreateRandomVehiclesEvent<X>>
    ...
    private CreateThread(final CreateRandomVehicleEventHandler<X, Y> eHandler,
      List<CreateRandomVehicleData> vehicleList,
      int startIndex,
      int endIndex) {
    ...
    public void run() {
      ...
      double limitBikes = this.eHandler.getResponsibleServer().
        getVehicleFuzzyManager().getPercentageValueBicycles();
      double limitCars = this.eHandler.getResponsibleServer().
        getVehicleFuzzyManager().getPercentageValueCars();
      // log.debug("CAR PERCENTAGE:"+limitCars);
      double limitMotorcycles = this.eHandler.getResponsibleServer().
        getVehicleFuzzyManager()
        .getPercentageValueMotorcycles();
      double limitTrucks = this.eHandler.getResponsibleServer().
        getVehicleFuzzyManager().getPercentageValueTrucks();
      ...
      for (int i = this.startIndex; i < this.endIndex; i++) {
        final CreateRandomVehicleData data = this.vehicleList.get(i);
        TrafficTrip trip = this.eHandler.getResponsibleServer().createTrip(
          this.eHandler.getResponsibleServer().getCityZone(),
          data.getVehicleInformation().getVehicleType());

        // Create vehicle
        final Vehicle<?> v = this.eHandler.createVehicle(data,
          trip);
        ...
        if (v.getData().getClass().equals(BicycleData.class)) {
          this.eHandler.getResponsibleServer().getVehicleFuzzyManager().
            increaseMaxBikes();
          counterBikes++;
          if (counterBikes <= limitBikes && trip.getStartTime() >= currentTime) {
            this.eHandler.scheduleVehicle(v,
              trip.getStartTime());
            countScheduledBike++;
          } else {
            this.eHandler.getResponsibleServer().getVehicleFuzzyManager().
      ...
      log.info(
        "Scheduled bikes: " + countScheduledBike + ", cars: " + countScheduledCar + ", motos: "
        + countScheduledMoto + ", trucks: " + countScheduledTruck);
      log.info(
        "Delayed bikes: " + countDelayedBike + ", cars: " + countDelayedCar + ", motos: "
        + countDelayedMoto + ", trucks: " + countDelayedTruck);
      log.info(
        "Spare bikes: " + countSpareBike + ", cars: " + countSpareCar + ", motos: " + countSpareMoto
        + ", trucks: " + countSpareTruck);
      log.info(
        "Without path bikes: " + countBikesWithoutPath + ", cars: " + countCarsWithoutPath + ", motos: "
        + countMotosWithoutPath + ", trucks: " + countTrucksWithoutPath);

      log.info(
        "Total fuzzy bikes: " + (countScheduledBike + countDelayedBike) + ", cars: "

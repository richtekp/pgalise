...
/**
 * Implementation of a TrafficGovernator using fuzzy rule logic. Input
 * variables, output variables, fuzzyfication, defuzzyfication, fuzzy rule set,
 * used operators for fuzzy rules and gravity method are defined in a separate
 * FCL-file (Fuzzy Control Language). To gain further information about FLC see {@linkplain http
...
public class FuzzyTrafficGovernor implements TrafficGovernor {
  ...
  /**
   * Path to FCL-file (FCL = Fuzzy Control Language) for {@link Car}s. To gain
   * further information about FLC see
  ...
  private abstract static class InputVariable {
    ...
    private InputVariable(final String variableName) {
      if (variableName == null) {
        throw new IllegalArgumentException(ExceptionMessages.
          getMessageForNotNull("variableName"));
      }
      this.variableName = variableName;
    }

    /**
     * sets this input variable for the passed {@link RuleBlock} at the passed
     * time.
     *
     * @param ruleBlock the rule block for which the input variable is to be set
     * @param cal the simulation time wrapped in a {@link Calendar}
     */
    private void set(final RuleBlock ruleBlock,
      final Calendar cal) {
      final double value = this.getValue(cal);
      // LOG.debug(this.variableName + ": " + value);
      ruleBlock.setVariable(this.variableName,
        value);
    }
     ...
     *
     * @param cal determines the time needed to get the value
   ...
   * passed "filePath" argument.
   *
   * @param fclFilePath path to FCL-file
   * @param weatherController the necessary weatherController for setting the
   * input variables
   * @exception IllegalArgumentException Is thrown if an argument is "null" or
   * the file path does not exist or the file is no valid FCL-File.
   */
  private FuzzyTrafficGovernor(final String fclFilePath) {
    ...
    try {
      this.fis = FIS.load(FuzzyTrafficGovernor.class.getResourceAsStream(
        fclFilePath),
        true);
     ...
     */
    this.inputLightIntensity = new InputVariable(
      FuzzyTrafficGovernor.INPUT_LIGHT_INTENSITY) {
        @Override
        protected double getValue(final Calendar cal) {
          return FuzzyTrafficGovernor.this.weatherController.getValue(
            WeatherParameterEnum.LIGHT_INTENSITY,
            cal.getTimeInMillis(),
            FuzzyTrafficGovernor.this.weatherController.getReferencePosition())
          .doubleValue();
        }
      };
    this.inputDayOfWeek = new InputVariable(
      FuzzyTrafficGovernor.INPUT_DAY_OF_WEEK) {
        @Override
        protected double getValue(final Calendar cal) {
          return (cal.get(Calendar.DAY_OF_WEEK) + 6) % 7;
        }
      };
    this.inputPrecipitationAmount = new InputVariable(
      FuzzyTrafficGovernor.INPUT_PRECIPITATION_AMOUNT) {
        @Override
        protected double getValue(final Calendar cal) {
          return FuzzyTrafficGovernor.this.weatherController.getValue(
    ...
    this.inputTemperature = new InputVariable(
      FuzzyTrafficGovernor.INPUT_TEMPERATURE) {
        @Override
        protected double getValue(final Calendar cal) {
          return FuzzyTrafficGovernor.this.weatherController.getValue(
    ...
    this.inputTimeOfDay = new InputVariable(
      FuzzyTrafficGovernor.INPUT_TIME_OF_DAY) {
        @Override
        protected double getValue(final Calendar cal) {
          return cal.get(Calendar.HOUR_OF_DAY) + (cal.get(Calendar.MINUTE) * (1D / 60D));
        }
      };
    this.inputWindStrength = new InputVariable(
      FuzzyTrafficGovernor.INPUT_WIND_STRENGTH) {
        @Override
        protected double getValue(final Calendar cal) {
          return FuzzyTrafficGovernor.this.weatherController.getValue(
            WeatherParameterEnum.WIND_STRENGTH,
            cal.getTimeInMillis(),
            FuzzyTrafficGovernor.this.weatherController.getReferencePosition())
          .doubleValue();
        }
  ...
  /**
   * Creates a default FuzzyTrafficGovernor from file
   * {@link src /main/resources/traffic.fcl}
   *
   * @exception IllegalArgumentException Is thrown if an argument is "null"
   */
   ...
   *
   * @param simTime the simulation time on which the traffic output is based
   * @param ruleBlock the rule block for the vehicle type
   * @param outputVariable the specialized output
   * @param inputVariables the input variables that have to be binded
   * @return the defuzzificated result
   */
  private double calcResult(final long simTime,
    final RuleBlock ruleBlock,
  ...
  /**
   * Returns the fuzzy calculated average percentage (0.0 - 1.0) of a
   * {@link Bus}' manning at a given simulation time.
   *
   * @param simTime the current simulation time
   * @return the fuzzy calculated average percentage (0.0 - 1.0) of a
   * {@link Bus}' manning
   */
  @Override
  public double getAverageBusManningPercentage(final long simTime) {
    final FunctionBlock functionBlock = this.fis.getFunctionBlock(
      FuzzyTrafficGovernor.FUNCTION_BLOCK_NAME_TRAFFIC);
    final RuleBlock ruleBlock = functionBlock.getFuzzyRuleBlock(
      FuzzyTrafficGovernor.RULE_BLOCK_NAME_BUSPASSENGERS);
    return this.calcResult(simTime,
      ruleBlock,
      FuzzyTrafficGovernor.OUTPUT_PERCENTAGE,
      this.inputTimeOfDay,
      this.inputDayOfWeek,
      this.inputSeason,
      this.inputTemperature,
      this.inputPrecipitationAmount,
      this.inputWindStrength);
  }

  /**
   * Returns the fuzzy calculated percentage (0.0 - 1.0) of active {@link Bike}s
  ...
  public double getPercentageOfActiveBicycles(final long simTime) {
    final FunctionBlock functionBlock = this.fis.getFunctionBlock(
      FuzzyTrafficGovernor.FUNCTION_BLOCK_NAME_TRAFFIC);
    final RuleBlock ruleBlock = functionBlock.getFuzzyRuleBlock(
      FuzzyTrafficGovernor.RULE_BLOCK_NAME_BICYCLES);
    return this.calcResult(simTime,
      ruleBlock,
      FuzzyTrafficGovernor.OUTPUT_PERCENTAGE,
      this.inputTimeOfDay,
  ...
  /**
   * Returns the fuzzy calculated percentage (0.0 - 1.0) of active {@link Car} s
   * at a given simulation time.
   *
   * @param simTime the current simulation time
  ...
  public double getPercentageOfActiveCars(final long simTime) {
    final FunctionBlock functionBlock = this.fis.getFunctionBlock(
      FuzzyTrafficGovernor.FUNCTION_BLOCK_NAME_TRAFFIC);
    final RuleBlock ruleBlock = functionBlock.getFuzzyRuleBlock(
      FuzzyTrafficGovernor.RULE_BLOCK_NAME_CARS);
    return this.calcResult(simTime,
      ruleBlock,
      FuzzyTrafficGovernor.OUTPUT_PERCENTAGE,
      this.inputTimeOfDay,
      this.inputDayOfWeek,
      this.inputLightIntensity,
      this.inputSeason,
      this.inputTemperature,
      this.inputPrecipitationAmount,
      this.inputWindStrength);
  ...
  /**
   * Returns the fuzzy calculated percentage (0.0 - 1.0) of active
   * {@link DefaultMotorcycle}s at a given simulation time.
   *
   * @param simTime the current simulation time
  ...
  public double getPercentageOfActiveMotorcycles(long simTime) {
    final FunctionBlock functionBlock = this.fis.getFunctionBlock(
      FuzzyTrafficGovernor.FUNCTION_BLOCK_NAME_TRAFFIC);
    final RuleBlock ruleBlock = functionBlock.getFuzzyRuleBlock(
      FuzzyTrafficGovernor.RULE_BLOCK_NAME_MOTORCYCLES);
    return this.calcResult(simTime,
      ruleBlock,
      FuzzyTrafficGovernor.OUTPUT_PERCENTAGE,
      this.inputTimeOfDay,
  ...
  /**
   * Returns the fuzzy calculated percentage (0.0 - 1.0) of active
   * {@link DefaultTruck}s at a given simulation time.
   *
   * @param simTime the current simulation time
  ...
  public double getPercentageOfActiveTrucks(final long simTime) {
    final FunctionBlock functionBlock = this.fis.getFunctionBlock(
      FuzzyTrafficGovernor.FUNCTION_BLOCK_NAME_TRAFFIC);
    final RuleBlock ruleBlock = functionBlock.getFuzzyRuleBlock(
      FuzzyTrafficGovernor.RULE_BLOCK_NAME_TRUCKS);
    return this.calcResult(simTime,
      ruleBlock,
      FuzzyTrafficGovernor.OUTPUT_PERCENTAGE,
      this.inputTimeOfDay,
  ...
  public void init(TrafficInitParameter initParameter) {
    if (this.weatherController.getStatus() == ControllerStatusEnum.INIT) {
      this.weatherController.init(initParameter);
    }
  }

  public void start(TrafficStartParameter startParameter) {
    if (this.weatherController.getStatus() == ControllerStatusEnum.INITIALIZED) {

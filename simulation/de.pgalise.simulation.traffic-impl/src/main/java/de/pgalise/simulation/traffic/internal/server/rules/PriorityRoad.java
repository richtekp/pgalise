...
public final class PriorityRoad extends TrafficRule {
	 ...
	 * @param trafficGraphExtensions service for accessing node's and edge's attributes
	 ...
	 */
	public PriorityRoad(final Node node, final RandomSeedService randomSeedService,
	...
	public PriorityRoad(final Node node, final RandomSeedService randomSeedService,
		...
		this(node, randomSeedService, trafficEdgeExtensions, 2000, 0.15);
	}
	 ...
	 */
	@Override
	protected void checkNode(final Node node) throws IllegalArgumentException, IllegalStateException {
	...
	public void register(final Vehicle vehicle, final Edge from, final Edge to, final TrafficRuleCallback callback)
		...
		if (from == to) {
			throw new UnsupportedOperationException("Turning around is not allowed here.");
		}
	...
	public void update(final SimulationEventList simulationEventList) {
		// Try to wipe out
		for (final Edge edge : this.inNode.keySet()) {
		...
		if (priorityCarsWaiting == 0) {
			for (final Edge edge : this.outNode.keySet()) {

 ...
 * class are used by the {@link CreateVehiclesEvent}. The first vehicle drives
 * from the given start node to the give target node. The start timestamp are
 * used here. The second vehicle drives from the given target node to the give
 * start node. For this trip, the end timestamp are used.
 *
 * @param <D>
 * @author Andreas
 * @version 1.0
...
public class CreateVehicleEventHandler<D extends VehicleData> extends AbstractVehicleEventHandler<D, CreateVehiclesEvent<D>> {
  ...
  private static final Logger log = LoggerFactory.getLogger(
    CreateVehicleEventHandler.class);
  ...
  public void handleEvent(CreateVehiclesEvent<D> event) {
    if (!event.getResponsibleServer().equals(this.getResponsibleServer())) {
      ...
      log.info("Processing CREATE_VEHICLES_EVENT: Vehicles=" + event.
        getVehicles().size());
    }
...
//				log.debug("No trip was specified. Creating random route...");
        trip = getResponsibleServer().createTrip(getResponsibleServer().
          getCityZone(),
...
//				log.debug("Just the start node was specified. Generating random target node...");
      ...
      if (this.getResponsibleServer().getCityZone().covers(
        GeoToolsBootstrapping.getGeometryFactory().createPoint(
          trip.getStartNode()))) {
        // Create vehicle

...
/**
 * This manager checks the informations of the TrafficGovernor and adapts the
 * amount of vehicle corresponding to the fuzzy logic.
 *
 * @author Lena
...
public class DefaultVehicleAmountManager implements VehicleAmountManager {
  ...
  private static Logger log = LoggerFactory.getLogger(
    DefaultVehicleAmountManager.class);
  /**
  ...
  /**
   * The Timebuffer is the range in which new added cars will start driving (in
   * min)
   */
   ...
   * steps is one simulation update.
   */
  ...
  /**
   * Handle the Vehicles with all their Trips. Delivers all planned Vehicles
   * with their Trip-information.
   */
  ...
  /**
   * Holds the current Date of the month. It will be generated out of the
   * timestamp.
   */
  ...
  /**
   * List of motorcycles which can be scheduled if the percentage of motorcycles
   * increases
   */
  private List<Vehicle<?>> spareMotorcycles;

  /**
   * List of bicycles which can be scheduled if the percentage of bicycles
   * increases
   */
  ...
  public DefaultVehicleAmountManager() {
  }

  /**
   * Constructor
   *
   * @param ts The TrafficServer, where this instance belongs to
   * @param tolerance the tolerance between old and new percentage of active
   * vehicles
   * @param updateSteps steps how often the TrafficFuzzyGoverner will be called
   * @param timebuffer the range in which new added cars will start driving (in
   * min)
  ...
  private void initialize(long timestamp) {
    ...
    this.percentageValueBicycles = fuzzy.
      getPercentageOfActiveBicycles(timestamp);
    this.percentageValueTrucks = fuzzy.getPercentageOfActiveTrucks(timestamp);
    this.percentageValueMotorcycles = fuzzy.getPercentageOfActiveMotorcycles(
      timestamp);
  ...
  public void checkVehicleAmount(long timestamp) {
    ...
    if (updateSteps == doneSteps) {
      ...
      if (this.oldPercentageValueCars < (fuzzy.getPercentageOfActiveCars(
        timestamp) - tolerance)
        || this.oldPercentageValueCars > (fuzzy.getPercentageOfActiveCars(
          timestamp) + tolerance)) {
        updateCarAmount(this.oldPercentageValueCars - fuzzy.
          getPercentageOfActiveCars(timestamp),
          timestamp);
        oldPercentageValueCars = fuzzy.getPercentageOfActiveCars(timestamp);
      }
      if (this.oldPercentageValueBicycles < (fuzzy.getPercentageOfActiveCars(
        timestamp) - tolerance)
        || this.oldPercentageValueBicycles > (fuzzy.
        getPercentageOfActiveBicycles(timestamp) + tolerance)) {
        updateBicycleAmount(this.oldPercentageValueBicycles - fuzzy.
          getPercentageOfActiveBicycles(timestamp),
          timestamp);
        oldPercentageValueBicycles = fuzzy.getPercentageOfActiveBicycles(
          timestamp);
      }
      if (this.oldPercentageValueTrucks < (fuzzy.getPercentageOfActiveTrucks(
        timestamp) - tolerance)
        || this.oldPercentageValueTrucks > (fuzzy.getPercentageOfActiveTrucks(
          timestamp) + tolerance)) {
        updateTruckAmount(this.oldPercentageValueTrucks - fuzzy.
          getPercentageOfActiveTrucks(timestamp),
          timestamp);
        oldPercentageValueTrucks = fuzzy.getPercentageOfActiveTrucks(timestamp);
      }
      if (this.oldPercentageValueMotorcycles < (fuzzy.
        getPercentageOfActiveMotorcycles(timestamp) - tolerance)
        || this.oldPercentageValueMotorcycles > (fuzzy.
        getPercentageOfActiveMotorcycles(timestamp) + tolerance)) {
        updateMotorcycleAmount(
          this.oldPercentageValueMotorcycles - fuzzy.
          getPercentageOfActiveMotorcycles(timestamp),
          timestamp);
        oldPercentageValueMotorcycles = fuzzy.getPercentageOfActiveMotorcycles(
          timestamp);
      }
  ...
  private void updateBicycleAmount(double addPercentage,
    long timestamp) {
    int changingAmount = (int) Math.floor(this.maxBikes * Math.
      abs(addPercentage));
    if (addPercentage > 0) {
  ...
  @SuppressWarnings({"rawtypes", "deprecation", "unchecked"})
  private void updateCarAmount(double addPercentage,
    long timestamp) {
    // Number of new Cars, percental to all cars. The concrete number will
    // be rounded.
    int changingAmount = (int) Math.
      floor(this.maxCars * Math.abs(addPercentage));
    if (addPercentage > 0) {
  ...
  @SuppressWarnings({"rawtypes", "deprecation", "unchecked"})
  private void updateMotorcycleAmount(double addPercentage,
    long timestamp) {
    int changingAmount = (int) Math.floor(this.maxMotorcycles * Math.abs(
      addPercentage));
    if (addPercentage > 0) {
      log.debug("changingAmount of motorcycles: " + changingAmount);
      int count = 1;
      for (Iterator<Vehicle<?>> i = this.spareMotorcycles.iterator(); i.
        hasNext();) {
        if (count > changingAmount) {
          break;
        }
  ...
  private void updateTruckAmount(double addPercentage,
    long timestamp) {
    int changingAmount = (int) Math.floor(this.maxTrucks * Math.abs(
      addPercentage));
    if (addPercentage > 0) {

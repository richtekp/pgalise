...
//
///**
// * A {@link NavigationEdge} which takes vehicles if the sum of security
// * distances of vehicles on this edge is less than the length of the edge
// * multiplied with the lane count. This means that the position of the vehicles
// * on the edge, the sequence in which they appear and driving behavior is
// * ignored.<br/>
// * Furthermore this edge assumes that vehicles don't change their velocity and
// * keeps the vehicles at least for the time they need to pass the length of the
// * edge. If the following edge can't take the vehicle, they stay on this edge,
// * but they're status remains as if they where driving on the edge.
// *
// * @param <N>
// * @param <E>
// * @author richter
...
// */
//@Entity
//public class ExtendedTrafficEdge<N extends TrafficNode, E extends NavigationEdge>
//  extends TrafficEdge {
//
//  private static final long serialVersionUID = 1L;
//  private final static Logger LOGGER = LoggerFactory.getLogger(
//    ExtendedTrafficEdge.class);
//  private int laneCount = 1;
//
//  /**
//   * initializes a <tt>DefaultNavigationEdge</tt> with 1 lane
//   */
//  public ExtendedTrafficEdge() {
//  }
//
//  public ExtendedTrafficEdge(Long id,
//    N source,
//    N target,
//    int laneCount) {
//    super(id,
...
//
//  @Override
//  public boolean takeVehicle(BaseVehicle<D> vehicle,
//    long timestamp) {
...
//      Measure<Integer, Length> securityDistance = BaseVehicle.
//        calculateSecurityDistance(vehicle0.getCurrentVelocity().getLeft());
//      securityDistanceSumInM += securityDistance.getUnit().getConverterTo(
//        SI.METER).convert(securityDistance.getValue());
//    }
//    double remainingSpace = getEdgeLength() * laneCount - securityDistanceSumInM;
//    double securityDistance = BaseVehicle.calculateSecurityDistance(vehicle.
//      getCurrentVelocity().getLeft()).to(SI.METER).getValue();
...
//    LOGGER.debug(String.format("updating %d vehicles",
//      getVehicles().size()));
...
//        LOGGER.debug(String.format("%s has no simulation steps left",
//          vehicle));
//        continue;
//      }
...
//          "requested simulation timestamp is behind last update timestamp of vehicle " + vehicle);
//      }
//      vehicle.getCurrentVelocity().setLeft(vehicle.getData().getMaxVelocity());
//      vehicle.getCurrentVelocity().setRight(timestamp);
//      List<E> passedEdges = moveOnRoute(vehicle,
//        new LongMeasure<>(deltaTime,
//          SI.MILLI(SI.SECOND)));
//      retValue.put(vehicle,
//        passedEdges);
//    }
...
//   * @return
//   */
//  private double edgeLengthPassedInM(E edge,
//    Coordinate position) {
//    Coordinate[] nearestPoints = DistanceOp.nearestPoints(edge.getEdgeLine(),
//      GeoToolsBootstrapping.getGEOMETRY_FACTORY().createPoint(new Coordinate(
//          position.getX(),
//          position.getY())));
//    if (nearestPoints.length != 2) {
//      throw new RuntimeException(new IllegalArgumentException(
//        "nearest points of a line and a position haves to be a pair of coordinates"));
//    }
//    if (AbstractNavigationEdge.distanceHaversineInM(nearestPoints[0],
//      nearestPoints[1]) > EDGE_LENGTH_PASSED_TOLERANCE) {
//      throw new IllegalArgumentException("position is not on edge");
//    }
//    double retValue = AbstractNavigationEdge.distanceHaversineInM(edge.
//      getSource().getGeoLocation(),
//      position);
//    return retValue;
//
...
//
//  /**
//   * moves the distance calculated from velocity and time over the nodes and
//   * edges and tells which nodes have been visited by the return value.<br/>
//   * Assumes that there're simulation steps left on the route.<br/>
//   * This doesn't modify the vehicles collection of the passed edge(s) in order
//   * to avoid {@link ConcurrentModificationException}, so this has to be done
//   * somewhere else, but vehicles are added to new edges.
//   *
//   * @param vehicle
//   * @param deltaTime
//   * @return a {@link List} with passed node (possibly empty) if the vehicle
//   * moved on the save {@link NavigationEdge} without reaching a node
//   */
//  public List<E> moveOnRoute(BaseVehicle<D> vehicle,
//    Measure<Long, Duration> deltaTime) {
//    LOGGER.debug(String.format("moving vehicle %s",
//      this.getId()));
//    long newTimestamp = vehicle.getPosition().getRight() + deltaTime.longValue(
//      SI.MILLI(SI.SECOND));
//    List<E> retValue = new LinkedList<>();
//    double distanceToDriveInM = BaseVehicle.velocityToDistanceInM(vehicle.
//      getVelocity(),
//      deltaTime);
//    double currentEdgeLength = vehicle.getCurrentEdge().getEdgeLength();
//    double currentEdgeRemainingLengthInM = currentEdgeLength;
//    double currentEdgeLengthPassedInM = edgeLengthPassedInM(vehicle.
//      getCurrentEdge().getLeft(),
//      vehicle.getCurrentPosition().getLeft());
...
//      "delta time: %f; current edge length: %f, current edge remaining length: %f, passed on edge: %f; distance to drive: %f",
//      deltaTime.doubleValue(SI.SECOND),
//      vehicle.getCurrentEdge().getLeft().getEdgeLength().doubleValue(SI.METER),
//      currentEdgeRemainingLengthInM,
...
//    while (distanceToDriveInM - currentEdgeRemainingLengthInM > 0) {
//      //go to next edge
//      LOGGER.debug(String.format("go to next edge (%d remaining)",
//        vehicle.getCurrentSimulationSteps().peek().getRoute().size()));
//      distanceToDriveInM -= currentEdgeRemainingLengthInM;
//      E passedEdge = vehicle.getCurrentSimulationSteps().peek().getRoute().
//        removeFirst();
//      retValue.add(passedEdge);
//      E newEdge = vehicle.getCurrentSimulationSteps().peek().getRoute().
//        peekFirst();
//      if (newEdge == null) {
//        vehicle.getCurrentPosition().setLeft(passedEdge.getTarget().
//          getGeoLocation());
...
//      currentEdgeRemainingLengthInM = vehicle.getCurrentEdge().getLeft().
//        getEdgeLength().to(SI.METER).getValue();
//    }
...
//      Vector2d edgeSourceVector = new Vector2d(vehicle.getCurrentEdge().
//        getLeft().getSource().getGeoLocation().getX(),
//        vehicle.getCurrentEdge().getLeft().getSource().getGeoLocation().getY());
//      Vector2d edgeTargetVector = new Vector2d(vehicle.getCurrentEdge().
//        getLeft().getTarget().getGeoLocation().getX(),
//        vehicle.getCurrentEdge().getLeft().getTarget().getGeoLocation().getY());
//
...
//      edgeVector.scale(distanceToDriveInM / vehicle.getCurrentEdge().getLeft().
//        getEdgeLength().doubleValue(SI.METER));
//
//      Vector2d currentPositionVector = new Vector2d(
//        vehicle.getCurrentPosition().getLeft().getX(),
//        vehicle.getCurrentPosition().getLeft().getY());
//      currentPositionVector.add(edgeVector);
//      newPosition = new Coordinate(currentPositionVector.getX(),
//        currentPositionVector.getY());
////			}else {
...
//
//      LOGGER.debug(String.format(
//        "new position: %s; distance to last position: %f; distance to edge source: %f",
//        newPosition,
//        AbstractNavigationEdge.distanceHaversineInM(
//          vehicle.getCurrentPosition().getLeft(),
//          newPosition),
//        AbstractNavigationEdge.distanceHaversineInM(vehicle.getCurrentEdge().
//          getLeft().getSource().getGeoLocation(),
//          newPosition)));
...
//    double sourceNodeDistance = AbstractNavigationEdge.distanceHaversineInM(
//      vehicle.getCurrentEdge().getLeft().getSource().getGeoLocation(),
//      vehicle.getCurrentPosition().getLeft());
//    if (sourceNodeDistance <= DefaultNavigationNode.NODE_RADIUS.intValue(
//      SI.METER)) {
//      vehicle.getCurrentNode().setLeft(vehicle.getCurrentEdge().getLeft().
//        getSource());
//      vehicle.getCurrentNode().setRight(newTimestamp);
//    } else {
//      double targetNodeDistance = AbstractNavigationEdge.distanceHaversineInM(
//        vehicle.getCurrentEdge().getLeft().getTarget().getGeoLocation(),
//        vehicle.getCurrentPosition().getLeft());
//      if (targetNodeDistance <= DefaultNavigationNode.NODE_RADIUS.intValue(
//        SI.METER)) {
//        vehicle.getCurrentNode().setLeft(vehicle.getCurrentEdge().getLeft().
//          getTarget());
//        vehicle.getCurrentNode().setRight(newTimestamp);
...
//
//  /**
//   * Moves on route for <tt>timeOnRoute</tt> (in 10^-3 s) and then leaves route
//   * and moves for <tt>timeOffRoute</tt> in <tt>azimuth</tt> or has to stay on
//   * route in the way can't be skipped (in this case
//   *
//   * @param awayFrom
//   * @param velocity
//   * @param direction
//   * @return
//   */
//  private Coordinate moveOffRoute(long timeOnRoute,
//    long timeOffRoute,
//    double azimuth) {

 ...
 * Considers only the first found suitable energy event.
 * It uses a PR-Tree to find suitable energy events for the requested positions.
 * @author Timo
 */
public class DefaultEnergyEventStrategy implements EnergyEventStrategyLocal {
	...
	public DefaultEnergyEventStrategy() {
		...
		this.changeEnergyConsumptionEventDataTree = new PRTree<>(NODE_MBR_CONVERTER, PR_TREE_BRANCH_FACTOR);
	...
	public void update(long timestamp, List<EnergyEvent> energyEventList) {
		...
		for(ChangeEnergyConsumptionEvent changeEvent : new ArrayList<>(this.currentChangeEvents)) {
		...
		for(ChangeEnergyConsumptionEvent changeEvent : new ArrayList<>(this.futureChangeEvents)) {
			if(changeEvent.getStartTimestamp() >= timestamp) {
	...
	public double getEnergyConsumptionInKWh(long timestamp,
		...
		for(DistanceResult<ChangeEnergyConsumptionEvent> result : results) {
			...
			if(result.getDistance() <= result.get().getMeasureRadiusInMeter() / 1000.0) {
				return result.get().changeEnergyConsumption(key, consumptionWithoutEvents);
			}
		}
	 ...
	 * @author Timo
	 */
	private static class NodeMBRConverter implements MBRConverter<ChangeEnergyConsumptionEvent> {
	 ...
	 * @author Timo
	 */
	private static class EventDistanceCalculator implements DistanceCalculator<ChangeEnergyConsumptionEvent> {
	 ...
	 * @author Timo
	 */
	private static class PRTreeEventFilter implements NodeFilter<ChangeEnergyConsumptionEvent> {

 ...
 * needed sensor ids to InfoSphere via TCP/IP. The InfoSphere IP and it's ports
 * can be changed in 'properties.props'.
 *
 * @author Timo
 */
public class DefaultGPSGateStrategy extends AbstractController<Event, TrafficStartParameter, TrafficInitParameter>
	implements GPSGateStrategy {

	private static final Logger log = LoggerFactory.getLogger(
		DefaultGPSGateStrategy.class);
	private static final long serialVersionUID = 1L;
	private String socketIP;
	private int socketPortRemoveKeys, socketPortAddKeys;
	/**
	 * Contains all sensors Map<Integer = sensor type, Set<Integer = Sensor id>>
	 */
	private Map<Integer, Set<Sensor>> allSensorTypeSensorIDMap;
	/**
	 * Contains only sensors which are known by InfoSphere Map<Integer = sensor
	 * type, Set<Integer = Sensor id>>
	 */
	private Map<Integer, Set<Sensor>> activeSensorTypeSensorIDMap;
	/**
	 * Contains only sensors which are not known by InfoSphere Map<Integer =
	 * sensor type, Set<Integer = Sensor id>>
	 */
	...
	public void handleGateMessage(Map<Integer, Double> gateInformationMap) throws UnknownHostException, IOException {
		...
		for (Entry<Integer, Double> entry : gateInformationMap.entrySet()) {
			if (SensorTypeEnum.getForSensorTypeId(entry.getKey()).equals(
				TrafficSensorTypeEnum.GPS)) {
				int activeSensorsForType = this.activeSensorTypeSensorIDMap.get(entry.
					getKey()).size();
				int notActiveSensorsForType = this.notActiveSensorTypeSensorIDMap.get(
					entry.getKey()).size();
				int allSensors = activeSensorsForType + notActiveSensorsForType;
				int newActiveValue = (int) ((allSensors) * entry.getValue());

				log.debug(
					"New Value: " + entry.getValue() + " newActiveValue: " + newActiveValue + " allSensors: " + allSensors + " notActiveSensors: " + notActiveSensorsForType + " active sensors: " + activeSensorsForType);

				if (newActiveValue > activeSensorsForType) {
					this.addAmountOfSensorsToInfoSphere(newActiveValue,
						entry.getKey());
				} else if (newActiveValue < activeSensorsForType) {
					this.removeAmountOfSensorsFromInfoSphere(newActiveValue,
						entry.getKey());
				}

				/* Update our map: */
				this.sensorTypePercentageMap.put(entry.getKey(),
					entry.getValue());
			}
		}
	}
	...
	public void createSensor(GpsSensor sensor) {
		if (sensor.getSensorType().equals(TrafficSensorTypeEnum.GPS)) {
			log.debug("create gps sensor. ID: " + sensor.getId() + " type: " + sensor.
				getSensorType());
			Set<Sensor> sensorCollection = this.allSensorTypeSensorIDMap.get(sensor.
				getSensorType().getSensorTypeId());
			if (sensorCollection == null) {
				sensorCollection = new HashSet<>();
				this.allSensorTypeSensorIDMap.put(sensor.getSensorType().
					getSensorTypeId(),
					sensorCollection);
			}
			...
			if (this.sensorTypePercentageMap.get(sensor.getSensorType().
				getSensorTypeId()) == 1.0) {
				List<Sensor> tmpSensorIDList = new LinkedList<>();
				tmpSensorIDList.add(sensor);
				this.addSensorIDsToInfoSphere(tmpSensorIDList);
				this.activeSensorTypeSensorIDMap.get(sensor.getSensorType().
					getSensorTypeId()).add(sensor);
			} else {
				this.notActiveSensorTypeSensorIDMap.get(sensor.getSensorType().
					getSensorTypeId()).add(sensor);
			}
		}
	}
	...
	public void createSensors(Set<GpsSensor> sensors)
		...
		for (Sensor sensor : sensors) {
			if (sensor.getSensorType().equals(TrafficSensorTypeEnum.GPS)) {
				log.debug(
					"create gps sensor. ID: " + sensor.getId() + " type: " + sensor.
					getSensorType());
				Set<Sensor> sensorCollection = this.allSensorTypeSensorIDMap.get(sensor.
					getSensorType().getSensorTypeId());
				if (sensorCollection == null) {
	...
	public void deleteSensor(GpsSensor sensor) {
		 ...
		 */
		if (sensor.getSensorType().equals(TrafficSensorTypeEnum.GPS)) {
			log.debug("delete gps sensor. ID: " + sensor.getId() + " type: " + sensor.
				getSensorType());
			this.allSensorTypeSensorIDMap.
				get(sensor.getSensorType().getSensorTypeId()).remove(sensor.getId());
			if (this.activeSensorTypeSensorIDMap.get(sensor.getSensorType().
				getSensorTypeId()).remove(sensor.getId())) {
				if (this.sensorTypePercentageMap.get(sensor.getSensorType()) < 1.0) {
					this.addAmountOfSensorsToInfoSphere(1,
	...
	public void deleteSensors(Set<GpsSensor> sensors)
		...
		for (Sensor sensor : sensors) {
			if (sensor.getSensorType().equals(TrafficSensorTypeEnum.GPS)) {
				log.debug(
					"delete gps sensor. ID: " + sensor.getId() + " type: " + sensor.
					getSensorType());
				this.allSensorTypeSensorIDMap.get(sensor.getSensorType().
					getSensorTypeId()).remove(sensor.getId());
				if (this.activeSensorTypeSensorIDMap.get(sensor.getSensorType().
					getSensorTypeId()).remove(sensor.getId())) {
					if (this.sensorTypePercentageMap.get(sensor.getSensorType().
						getSensorTypeId()) < 1.0) {
						Integer amount = sensorTypeAddNewMap.get(sensor.getSensorType().
							getSensorTypeId());
						if (amount == null) {
							amount = 0;
						}
						sensorTypeAddNewMap.put(sensor.getSensorType().getSensorTypeId(),
							amount + 1);
					}
				} else {
					this.notActiveSensorTypeSensorIDMap.get(sensor.getSensorType().
		...
		for (Entry<Integer, Integer> entry : sensorTypeAddNewMap.entrySet()) {
			this.addAmountOfSensorsToInfoSphere(entry.getValue(),
				entry.getKey());
		}
	}
	...
	private void resetAllAttributes() {
		...
		this.socketPortRemoveKeys = Integer.valueOf(properties.getProperty(
			"InfoSphereGateMessageRemoveKeysPort"));
		this.socketPortAddKeys = Integer.valueOf(properties.getProperty(
			"InfoSphereGateMessageAddKeysPort"));
		...
		this.allSensorTypeSensorIDMap.put(TrafficSensorTypeEnum.GPS.
			getSensorTypeId(),
			new HashSet<Sensor>());
		this.activeSensorTypeSensorIDMap.put(TrafficSensorTypeEnum.GPS.
			getSensorTypeId(),
			new HashSet<Sensor>());
		this.notActiveSensorTypeSensorIDMap.put(TrafficSensorTypeEnum.GPS.
			getSensorTypeId(),
			new HashSet<Sensor>());
		this.sensorTypePercentageMap.
			put(TrafficSensorTypeEnum.GPS.getSensorTypeId(),
				1.0);
	}
	...
	private void addSensorIDsToInfoSphere(Collection<Sensor> sensorIDCollection) {
		...
		try {
			socket = new Socket(this.socketIP,
				this.socketPortAddKeys);
	 ...
	 * @throws IOException
	 */
	private void removeSensorIDsFromInfoSphere(
		Collection<Sensor> sensorIDCollection) throws IOException {
		Socket socket = null;
		DataOutputStream outStream = null;
		try {
			socket = new Socket(this.socketIP,
				this.socketPortRemoveKeys);
			outStream = new DataOutputStream(socket.getOutputStream());
	...
	/**
	 * Adds the specified amount of sensors to InfoSphere. Removes them also from
	 * not active map and adds them to active map.
	 *
	 * @param amount
	 * @param sensorType
	 */
	private void addAmountOfSensorsToInfoSphere(int amount,
		int sensorType) {
		Set<Sensor> sensorIDs = new HashSet<>();
		Set<Sensor> activeSensorSet = this.activeSensorTypeSensorIDMap.get(
			sensorType);
		Set<Sensor> notActiveSensorSet = this.notActiveSensorTypeSensorIDMap.get(
			sensorType);
	...
	/**
	 * Removes the given amount of sensors from InfoSphere. Removes them also from
	 * active map and adds them to not active map.
	 *
	 * @param amount
	 * @param sensorType
	 * @throws IOException
	 */
	private void removeAmountOfSensorsFromInfoSphere(int amount,
		int sensorType) throws IOException {

...
public class PRTree<T> implements Serializable {
  ...
  private static final long serialVersionUID = 8151213305855731487L;
  ...
  public void load(Collection<? extends T> data) {
    ...
    numLeafs = data.size();
    LeafBuilder lb = new LeafBuilder(converter.getDimensions(),
      branchFactor);

    List<LeafNode<T>> leafNodes
      = new ArrayList<>(estimateSize(numLeafs));
    lb.buildLeafs(data,
      new DataComparators<>(converter),
      new LeafNodeFactory(),
      leafNodes);
    ...
    while (nodes.size() > branchFactor) {
      ...
      lb.buildLeafs(nodes,
        new InternalNodeComparators<>(converter),
  ...
  public MBR2D getMBR2D() {
    ...
    return new SimpleMBR2D(mbr.getMin(0),
      mbr.getMin(1),
      mbr.getMax(0),
      mbr.getMax(1));
  }
  ...
  /**
   * Find all objects that intersect the given rectangle. Note, this find method
   * will only use two dimensions, no matter how many dimensions the PRTree
   * actually has.
   *
   * @param xmin the minimum value of the x coordinate when searching
   * @param ymin the minimum value of the y coordinate when searching
  ...
  public Iterable<T> find(double xmin,
    ...
    double xmax,
    double ymax) {
    return find(new SimpleMBR(xmin,
      xmax,
      ymin,
      ymax));
  }
   ...
   * @return A List of DistanceResult with up to maxHits results. Will return an
   * empty list if this tree is empty.
   */
  public List<DistanceResult<T>> nearestNeighbour(DistanceCalculator<T> dc,
    ...
    NearestNeighbour<T> nn
      = new NearestNeighbour<>(converter,
        filter,
        maxHits,
        root,
        dc,

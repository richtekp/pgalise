...
public class TimeSeriesChart extends ApplicationFrame {
   ...
   *
   * @param list List of timeseries object
   * @return Dataset
   */
   ...
   *
   * @param map Map with weather objects
   ...
   * @return TimeSerie
   */
  public static TimeSeries getTimeSerie(String title,
    List<AbstractStationData> list,
    WeatherParameterEnum key) {
    ...
    for (AbstractStationData weather : list) {
      try {
        Timestamp time = new Timestamp(
          weather.getMeasureTime().getTime() + 3600000);
        s1.add(new Millisecond(time),
          TimeSeriesChart.getValue(key,
            weather));
      } catch (Exception ex) {
        // Do nothing
      }
   ...
   *
   * @param title Title
   * @param map List of weather objects
   * @param key WeatherParameterEnum
   * @return TimeSerie
   */
  public static TimeSeries getTimeSerie(String title,
    WeatherMap map,
    WeatherParameterEnum key) {
    return TimeSeriesChart.getTimeSerie(title,
      TimeSeriesChart.getList(map),
      key);
  }
   ...
   */
  private static double getValue(WeatherParameterEnum key,
  ...
  public TimeSeriesChart(String title,
    ...
    this(title,
      date,
      unit,
      TimeSeriesChart.createDataset(series));
  }
   ...
   * @param series List with TimeSeries
   */
  public TimeSeriesChart(String title,
    long date,
    ...
    this(title,
      date,
      TimeSeriesChart.getValueUnit(parameter),
      series);
  }
   ...
   * @param dataset XYDataset
   */
  public void repaintChart(String title,
    String date,
    String unit,
    XYDataset dataset) {
    // Change attributes
  ...
  private void setChart(XYDataset dataset) {
    ...
    // Create new chart
    JFreeChart chartNew = ChartFactory.createTimeSeriesChart(
    ...
    chartNew.setPadding(new RectangleInsets(0.0,
      0.0,
      0.0,
      0.0));
    ...
    plot.setAxisOffset(new RectangleInsets(5.0,
      5.0,
      5.0,
      5.0));

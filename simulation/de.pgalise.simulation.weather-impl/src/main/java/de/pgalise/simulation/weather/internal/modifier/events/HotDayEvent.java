...
/**
 * Changes the weather for a hot day ({@link WeatherMapModifier} and
...
public class HotDayEvent extends WeatherDayEventModifier {
  ...
  public void deployChanges() {
    ...
    float maxDifference = this.maxValue - max.getTemperature().floatValue(
      SI.CELSIUS);

    HotDayEvent.log.debug(
      "Max. value of event (" + max.getMeasureDate() + "): " + this.maxValue
    ...
    if (maxDifference > 0) {
       ...
       */
      // Event limits
      long minTime = this.getMinHour(max.getMeasureTime(),
        this.getEventDuration());
      HotDayEvent.log.debug(
        "Min. time of event: " + new Date(minTime) + " (Duration: " + this.
        getEventDuration()
        + ")");
      long maxTime = this.getMaxHour(max.getMeasureTime(),
        this.getEventDuration());
      HotDayEvent.log.debug(
  ...
  protected void initDecorator() {
    if (this.getProps() != null) {
      ...
      this.max_value = Integer.parseInt(this.getProps().getProperty(
        "hotday_max_value"));
      this.max_value_range = Integer.parseInt(this.getProps().getProperty(
        "hotday_max_value_range"));

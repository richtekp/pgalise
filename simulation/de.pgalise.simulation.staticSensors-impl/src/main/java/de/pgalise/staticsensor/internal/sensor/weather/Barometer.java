...
public final class Barometer extends WeatherSensor {
	...
	public void transmitUsageData(SimulationEventList eventList) {
		...
		// Get value
		this.airPressure = (this.getWeatherController().getValue(WeatherParameterEnum.AIR_PRESSURE,
				eventList.getTimestamp(), this.getPosition())).doubleValue();

		// Interfere
		this.airPressure = this.getInterferer().interfere(this.airPressure, this.getPosition(),
				eventList.getTimestamp());

		// transmit value
		this.getOutput().transmitDouble(this.airPressure);

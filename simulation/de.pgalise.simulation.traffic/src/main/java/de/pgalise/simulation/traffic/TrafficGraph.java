 ...
 */
public class TrafficGraph<N extends TrafficNode, E extends TrafficEdge<N,E>> extends DefaultDirectedGraph<N, E> implements Identifiable {
	private static final long serialVersionUID = 1L;
	
	private Long id;

	public TrafficGraph(Class<E> edgeClass) {
		super(new NavigationEdgeFactory<>(edgeClass));
	}
	...
	public Identifiable getElementClosestTo(Coordinate position, double distanceTolerance) {
		...
		NavigationNode minDistanceNode = null;
		double minDistance = Double.MAX_VALUE;
		for(NavigationNode node : this.vertexSet()) {
		...
		NavigationEdge minDistanceEdge = null;
		for(NavigationEdge edge : this.edgeSet()) {
	...
	public NavigationNode getNodeClosestTo(Coordinate position) {
		return getNodeClosestTo(position,
			Integer.MAX_VALUE);
	}
	
	public NavigationNode getNodeClosestTo(Coordinate position, int distanceTolerance) {
	...
	private static class NavigationEdgeFactory<Y extends TrafficNode, X extends TrafficEdge<Y,X>> implements EdgeFactory<Y, X> {

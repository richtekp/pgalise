...
public class Roundabout extends AbstractTrafficRule {
	 ...
	 */
	private final Map<TrafficEdge , Map<TrafficEdge , Integer>> edgeToEdgeTimes = new HashMap<>();
	 ...
	 */
	public Roundabout(final TrafficNode node, TrafficGraph graph, final RandomSeedService randomSeedService,
	 ...
	 */
	public int getMillis(final TrafficEdge from, final TrafficEdge to) throws IllegalArgumentException, IllegalStateException {
		if (from == null) {
		...
		final Map<TrafficEdge, Integer> map = this.edgeToEdgeTimes.get(from);
		if (map == null) {
	...
	public int getNumberOfVehiclesWaiting() {
		int result = 0;
		for (final TrafficEdge key : this.vehiclesWaiting.keySet()) {
	 ...
	 */
	public int getNumberOfVehiclesWaiting(final TrafficEdge from) throws IllegalArgumentException, IllegalStateException {
		if (from == null) {

...
/**
 * clearify the function of the class and find a mechanism to deal with non-unique ids of bus stops (see comments)
 * @author Lena
 */
public class VWGGTFSCreator {

	private List<String> csvFiles;
	private List<BusStopInformation> busStopInformations;
	...
	public void parseVWGCSVFilesToStopTimes(String busLine) throws IOException {
		...
		busStopInformations = new ArrayList<>();
		...
		for (int i = 0; i < csvFiles.size(); i++) {
			...
			while ((rowAsTokensStops = csvReaderStops.read()) != null) {
				...
				BusStop busStop = entityManager.find(BusStop.class,
					Long.valueOf(stopsStopId));
			...
			while ((rowAsTokens = csvReader.read()) != null) {
				...
				for (BusStop stop : stopsList) {
					if (busStopName.equals(stop.getStopName())) {
						stop_id = stop.getId();
					}
				}
	...
	public void parseVWGCSVFiles(BusRoute busLine) throws IOException {
		...
		for (int i = 0; i < csvFiles.size(); i++) {
			...
			while ((rowAsTokens = csvReader.read()) != null) {
				...
				BusStop busStop = (BusStop) entityManager.createQuery("SELECT x.stopName FROM BusStop x where x.stopName = '%s'"+busStopName+"'").getSingleResult(); //@TODO: make better query
				busStopInformations.add(new BusStopInformation(busStop,
					cal.getTimeInMillis()));
			}

	...
	var CreateVehiclesEvent = (function() {
		function CreateVehiclesEvent(path) {
        	...
        	for(var vid in path.vehicles) {
        		...
        		vehicleList.push(
        			new model.VehicleInformation(
	        			vehicle,
	        			new model.TrafficTrip(
	        				path.start.nodeId,
	        				new model.GeoLocation(path.start.latitude, path.start.longitude),
	        				path.end.nodeId,
	        				new model.GeoLocation(path.end.latitude, path.end.longitude),
	        				vehicle.startTime
	        			)
        			)
        		);

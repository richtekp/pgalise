 ...
 * enter the control center with more than one user. This servlet can also receive update and stop information from the simulation 
 * and sends it to the control center user.
 * 
 * @author Timo
 */
public class CCWebSocketServlet extends HttpServlet {
	 ...
	 * It will be called on it's own after construct.
	 */
	@PostConstruct
	...
	/**
	 * Removes an online user. Another user can enter the control center after this method.
	 * @param controlCenterUserWebSocket
	 */
	public static synchronized void removeUser(CCWebSocketUser controlCenterUserWebSocket) {
	 ...
	 * 
	 * @param ip
	 *            the simulation controller is running on this server. Ip/address : port
	 * @param port
	 *            port of the server
	 * @return
	...
	public SimulationController getSimulationController(String address) throws NamingException {
		...
		if (ip.equals("127.0.0.1") || ip.equals("localhost")) {
			...
			return (SimulationController) ctx.lookup("de.pgalise.simulation.SimulationControllerLocal");
		} else {
			props.put(Context.INITIAL_CONTEXT_FACTORY, "org.apache.openejb.client.InitialContextFactory");
			props.put(Context.PROVIDER_URL, "http://" + ip + ":" + port + "/tomee/ejb");
			Context ctx = new InitialContext(props);

			return (SimulationController) ctx.lookup("de.pgalise.simulation.SimulationController");
		}
	}

...
//		}
		...
		RandomSeedService randomSeedService = EasyMock.createNiceMock(
					...
					case CAR:
						...
						for (Sensor<?, ?> sensorHelper : createRandomVehicleData.
							...
							if (sensorHelper instanceof GpsSensor) {
								...
								DefaultRandomDynamicSensorServiceTest.newSensorIDs.add(
												sensorHelper);
								break;
							}
						}
	...
	public void testIDs() {
		...
		Assert.assertEquals(DefaultRandomDynamicSensorServiceTest.usedSensorIDs.
						size()
																+ DefaultRandomDynamicSensorServiceTest.newSensorIDs.
						size(), allIDs.size());
		...
		Assert.assertEquals(DefaultRandomDynamicSensorServiceTest.usedUUIDs.size()
																+ DefaultRandomDynamicSensorServiceTest.newUUIDs.
						size(), allUUIDs.size());
	}

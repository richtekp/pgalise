...
import de.pgalise.weathercollector.model.ServiceDataCurrent;
 ...
 */
public class DatabaseManager implements WeatherStationSaver, WeatherServiceSaver {
	...
	private DatabaseManager() {
		...
		// Read database props
		try (InputStream propInFile = DatabaseManager.class.getResourceAsStream(DATABASE_FILE_PATH)) {
	...
	public boolean saveStationDataSet(Set<StationData> list) throws SaveStationDataException {
		...
		for (StationData station_data : list) {
			...
			if (actTime <= lastTime) {
				Log.writeLog("Holen von Daten abgebrochen! Datensaetze bis zum " + station_data.getDate() + " - "
		...
		Log.writeLog(count + " Datensaetze wurden gespeichert.", Level.INFO);
	...
	private void saveCurrentWeather(int cityID, ServiceDataCurrent service_data) {
		...
		Log.writeLog("Datensatz fuer Stadt " + cityID + " vom " + service_data.getDate() + " gespeichert.", Level.INFO);
	}
	...
	private void saveForecastWeather(int cityID, Set<ServiceDataForecast> service_data) {
		...
		for (ServiceDataForecast data : service_data) {
			...
			Log.writeLog("Datensatz fuer Stadt " + cityID + " vom " + data.getDate() + " gespeichert.", Level.INFO);

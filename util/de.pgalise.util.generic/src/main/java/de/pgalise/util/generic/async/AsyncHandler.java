...
/**
 * AsynHandler runs commands asynchronously and calls the callback function when
 * all proceedings have been finished.
 *
 * @author mustafa
 */
public abstract class AsyncHandler {
	...
	/**
	 * Sets the DelegateFunction. A DelegateFunction represents a specific task.
	 * For each task the AsyncHandler starts a asynchronous routine for the
	 * processing. Delegate function will be removed as soon as it has finished
	 * its operation.
	 *
	 * @param f task to be done by the asynchronous routine
	 */
	...
	/**
	 * Sets the callback function. It is called when all asynchronous routines
	 * finished their job.
	 *
	 ...
	 *
	 * @param n n=-1 when finished the job
	 */
	...
	/**
	 * Creates an asynchronous routine and defines what it looks like in
	 * particular. A ThreadHandler for example could implement here a asynchronous
	 * routine as a separate thread. Do not forget to call incRunningWorker(-1)
	 * when the routine finished his task.
	 *
	 * @param d task to be done by the asynchronous routine
	 */
